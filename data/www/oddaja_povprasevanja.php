...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // 1. Pridobivanje podatkov iz obrazca
    ...
    try {
        $pdo->beginTransaction(); // Začnemo transakcijo, da se vse izvede varno
       ...
       $stmt_check = $pdo->prepare("SELECT ID_uporabnik FROM Uporabnik WHERE email = :email");
        ...
        if ($existing_user_id) {
            $nov_user_id = $existing_user_id; // Uporabimo obstoječega
        } else {
            // Ustvarimo novega
        ...
        // 4. PRIDOBITEV IMENA ŽIVALI (za mail)
        ...
        // Tukaj se podatek dejansko shrani v bazo
        $stmt_povp->execute([
        ...
        $pdo->commit(); // Potrdimo vnose v bazo

        // 6. POŠILJANJE STILIZIRANEGA MAILA
        ...
        $mail->Password   = 'abkdfepyfjqynjbr'; // App Password
        ...
        // --- MAIL 2: OBVESTILO ZAVETIŠČU (TEBI) ---
        $mail->clearAddresses(); // Pobrišemo prejšnjega prejemnika
        $mail->addAddress($moj_gmail, 'Admin ShelterCompass'); // Prejemnik si ti
        $mail->addReplyTo($user_email, $full_name); // Če klikneš 'Odgovori', pišeš uporabniku
